					...
					if(is_working){
						...
						try{
							...
							while(e_data1->Read() == true)
							{
								...
								//employee_name=e_data1->GetString(1);
								DateTime subjectdate=PlannerDate->Value;
								//MessageBox::Show("Sub date"+subjectdate);
								//MessageBox::Show("leave start"+leave_start);
								TimeSpan diff1= subjectdate.Subtract(leave_start);
								TimeSpan diff2= leave_end.Subtract(subjectdate);
								//MessageBox::Show(diff1 + "the differences" + diff2);
